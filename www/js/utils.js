...
<style>
  ...
  updateURL: (path) => {
    if (!path) return window.history.replaceState(null, null, '.')
    const p = window.location.protocol
    const h = window.location.host
    // window.location = `${p}//${h}/${path}`
    window.history.replaceState(null, null, `${p}//${h}/${path}`)
    window.utils.url = {
    ...
    if (WIDGETS['privacy-policy']) WIDGETS.open('privacy-policy')
      ...
      WIDGETS.create({
        ...
        innerHTML: `
          ...
          <p>The second exception happens if and when you connect your GitHub to allow me to save any work you make here in the studio as repositories in your GitHub account. In order to communicate with GitHub's servers I'll act as an intermediary, storing your project data on my server temporarily as your work and passing it along to GitHub for permanent storage if/when you choose to do so (again, refer to my open source code on <a href="https://github.com/netizenorg/netnet.studio" target="_blank">GitHub</a> for proof).</p>
